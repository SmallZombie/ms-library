const Popup = {
     ...
     * @param {int} stayTime 停留时间(秒)，默认为7，0为永远停留(不自动消失)
     */
    info: (message, type = 0, stayTime = 0) => {
        if (!message) return;
        ...
        temp.querySelector('.close').addEventListener('click', () => {
            Popup.info_close(temp);
        })

        // 弹出
        document.body.appendChild(temp);
        Popup.allDia.push(temp);
        ...
        // 自动关闭
        if (stayTime !== 0) setTimeout(() => Popup.info_close(temp), stayTime * 1000);
    },
    info_close: (el) => {
        if (el) {
            Popup.allDia = Popup.allDia.filter(item => item !== el);
            el.style.top = '-100px';
