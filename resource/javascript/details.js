 ...
 * 传入玩家id，返回image对象
 * @param {*} id 玩家id
 * @returns 玩家id对应的image对象
 */
function getImage(id) {
    return (
        // 先判断id是否为空，然后再判断window.namemc是否存在，再判断images属性是否存在，最后判断images[id]，为true就返回对应的image对象
        id && window.namemc && window.namemc.images && window.namemc.images[id]
    );
}
/**
...
function updateRenderButton() {
    if (skin3d.renderFormTheta)
        skin3d.renderFormTheta.value = skin3d.theta
            .toFixed(2)
            .replace(/\.?0+$/, "");
    if (skin3d.renderFormPhi)
        skin3d.renderFormPhi.value = skin3d.phi.toFixed(2).replace(/\.?0+$/, "");
    if (skin3d.renderFormTime)
        skin3d.renderFormTime.value = skin3d.time.toFixed(2).replace(/\.?0+$/, "");
}
/**
 * 切换皮肤动画播放状态
 */
function toggleSkinAnimate() {
    ...
    if (skin3d.animate) { // 播放
        skin3d.animateButton.classList.add('playing');
        skin3d.animateButton.querySelector('span').innerText = '暂停';
        skin3d.animateButton.title = '暂停播放';
        ...
        skin3d.animateButton.classList.remove('playing');
        skin3d.animateButton.querySelector('span').innerText = '播放';
        skin3d.animateButton.title = '开始播放';
...
function enableSkinRotation() {
    ...
    skin3d.canvas.addEventListener("mousedown", (e) => {
        e.preventDefault();
...
/**
 * 渲染皮肤，// TODO 目前不清楚会循环渲染(循环触发该方法)还是只会渲染一次(触发一次)
 * @returns 
 */
function renderSkin() {
    skin3d.id = skin3d.canvas.getAttribute("data-id"); // 从canvas属性中取出data-id(玩家id)
    skin3d.cape = skin3d.canvas.getAttribute("data-cape"); // ...取出data-cape(披风id)
    skin3d.flip = skin3d.canvas.getAttribute("data-flip") === "true"; // ...取出data-flip(翻转?)
    if (typeof skin3d.theta !== "number") { // 这里是给各个属性设置值的地方，可能也有数据矫正(因为判断是否初始化用undefined就可以了，这里还判断了类型)
        skin3d.theta = skin3d.canvas.getAttribute("data-theta") // 如果canvas属性data-theta存在，
            ? parseFloat(skin3d.canvas.getAttribute("data-theta")) // 就将这个属性转为浮点数，赋值给skin3d.theta
            : 30; // 否则设置为30(应该是默认值)
    }
    if (typeof skin3d.phi !== "number") { // 类似上面
        skin3d.phi = skin3d.canvas.getAttribute("data-phi")
            ? parseFloat(skin3d.canvas.getAttribute("data-phi"))
            : 21;
    }
    if (typeof skin3d.time !== "number") {
        skin3d.time = skin3d.canvas.getAttribute("data-time")
            ? parseFloat(skin3d.canvas.getAttribute("data-time"))
            : skin3d.flip ? 270 : 90;
    }
    let key = `${skin3d.id}:${skin3d.cape}:${skin3d.slim}`; // 这里使用玩家id:披风id:皮肤粗细，拼了一个key出来
    if (!skin3d.modelCache[key]) { // 用这个key去modelCache找value，如果找不到就
        let skin = toCanvas(getImage(skin3d.id)); // 根据玩家id拿到image对象，toCanvas最终获得一个canvas对象
        let cape = toCanvas(getImage(skin3d.cape)); // ...         披风的image对象，...
        if (!skin || (skin3d.cape && !cape)) return; // 如果skin是空的，或者有披风但是没有披风的canvas对象就直接返回
        skin3d.modelCache[key] = buildSkinModel(
            skin,
            cape,
            skin3d.slim,
            skin3d.flip
...
function initializeSkin(id) {
    if (!skin3d.canvas) return; // 如果找不到canvas就直接返回
    let skinId = skin3d.canvas.getAttribute("data-id"); // 从canvasDOM中取回属性值data-id(玩家id)
    let capeId = skin3d.canvas.getAttribute("data-cape"); // 从canvasDOM中取回属性值data-cape(披风id)
    if (id !== skinId && id !== capeId) return; // 如果传入的玩家id与两种id都不匹配就直接返回
    if (!getImage(skinId) || (capeId && !getImage(capeId))) return; // 如果根据玩家id拿不到image对象，或有披风id且拿不到披风image对象，就直接返回
    renderSkin();
    // 启用皮肤旋转(鼠标拖动旋转)
    // 这里我猜不一开始就启用皮肤旋转是因为canvas没渲染完，此时处理旋转逻辑的话会出错，
    // 到这里上面已经渲染皮肤了，所以不会出问题
    enableSkinRotation();
}
// 从玩家id.js里传过来一个对象，键为玩家id，值为玩家皮肤base64编码
async function nmci(pngs, slim = false) {
    // 准备初始数据
    skin3d.animate = false;
    skin3d.slim = +slim;

    if (!window.namemc) window.namemc = {}; // 检查window中是否有namemc这个全局变量，没有的话就将其初始化为一个空对象
    if (!window.namemc.images) window.namemc.images = {}; // 检查window中...这个属性，没有的话就...
    let id = 'temp';
    if (!window.namemc.images[id]) { // 如果images里已经有这个id了就跳过
        window.namemc.images[id] = await toImage(pngs); // 否则将toImage反回的image对象赋值给images[id]
        initializeSkin(id); // 初始化皮肤
        drawCape(id); // 绘制披风
    }
}
...
function formatTimezoneOffset(time) {
    ...
    return `(UTC${sign}${hourOffset.toString().padStart(2, "0")}:${minuteOffset
        .toString()
        .padStart(2, "0")})`;
}
function localizeTimes() {
    ...
    document.querySelectorAll(".title-time").forEach((e) =>
        e.setAttribute(
            "title",
            [e.getAttribute("title"), e.getAttribute("data-title-2")]
                .filter((t) => !!t)
                .map((t) => new Date(t))
